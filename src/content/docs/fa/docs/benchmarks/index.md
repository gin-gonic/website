...
> **داده‌های به‌روز:** این بنچمارک‌ها در مارس ۲۰۲۶ با استفاده از Gin v1.12.0 و Go 1.25.8 جمع‌آوری شده‌اند. برای مشاهده نتایج به‌روز بنچمارک، مخزن [go-http-routing-benchmark](https://github.com/gin-gonic/go-http-routing-benchmark) را ببینید.
...
جدول زیر تمام مسیریاب‌ها را بر اساس **توان عملیاتی GitHub API** (۲۰۳ مسیر، تمام متدها) رتبه‌بندی می‌کند که بهترین نمایانگر بارهای کاری واقعی مسیریابی است. _مقدار ns/op کمتر بهتر است._
...
- Gin، BunRouter و Echo در رده بالا قرار دارند — همگی بدون تخصیص حافظه heap عمل می‌کنند و کل GitHub API را در حدود ۱۰ میکروثانیه مسیریابی می‌کنند.
- **HttpRouter** همچنان بسیار سریع است اما به ازای هر مسیر پارامتردار ۱ تخصیص حافظه دارد (۱۶۷ تخصیص برای ۲۰۳ مسیر).
- Fiber نیز بدون تخصیص حافظه عمل می‌کند، اما زیرساخت بنچمارک مبتنی بر fasthttp آن، سربار بازنشانی در هر تکرار اضافه می‌کند — مقایسه مستقیم با مسیریاب‌های net/http نیازمند احتیاط است.
- **GorillaMux** و **GoRestful** از نظر قابلیت‌ها غنی هستند اما مرتبه‌هایی کندتر عمل می‌کنند و برای برنامه‌های حساس به تأخیر مناسب نیستند.

> **نکته درباره Fiber:** بنچمارک‌های Fiber از `fasthttp.RequestCtx` با بازنشانی در هر تکرار استفاده می‌کنند که سربار ثابتی اضافه می‌کند که در بنچمارک‌های net/http وجود ندارد. مقایسه‌های Fiber-با-Fiber معتبر هستند؛ مقایسه‌های بین فریمورک‌ها باید با دقت تفسیر شوند.

---

## مصرف حافظه

حافظه مورد نیاز برای بارگذاری ساختار مسیریابی (کمتر بهتر است). بر اساس بایت به ترتیب صعودی مرتب شده است.
...
مسیریابی تمام ۲۰۳ نقطه پایانی GitHub API در هر عملیات.
...
مسیریابی تمام ۱۳ نقطه پایانی Google+ API در هر عملیات.
...
مسیریابی تمام ۲۶ نقطه پایانی Parse API در هر عملیات.
...
### مسیرهای ثابت (۱۵۷ مسیر)

مسیریابی تمام ۱۵۷ مسیر ثابت در هر عملیات. شامل http.ServeMux به عنوان خط مبنا.
...
مسیر: `/user/:name` — درخواست: `GET /user/gordon`
...
مسیر: `/:a/:b/:c/:d/:e` — درخواست: `GET /test/test/test/test/test`
...
مسیر: `/:a/:b/.../:t` (۲۰ بخش) — درخواست: `GET /a/b/.../t`
...
مسیر: `/user/:name` با نوشتن پاسخ — درخواست: `GET /user/gordon`
