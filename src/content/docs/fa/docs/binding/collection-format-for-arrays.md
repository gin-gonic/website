...
می‌توانید نحوه تقسیم مقادیر لیست برای فیلدهای slice/آرایه را با استفاده از تگ struct `collection_format` با اتصال فرم کنترل کنید.

فرمت‌های پشتیبانی شده (نسخه v1.11+):
...
مقادیر پیش‌فرض برای مجموعه‌ها (نسخه v1.11+):

- از `default` در تگ `form` برای تنظیم مقادیر بازگشتی استفاده کنید.
...
همچنین ببینید: مثال "مقادیر پیش‌فرض برای فیلدهای فرم".
