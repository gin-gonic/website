...
フォームバインディングで`collection_format`構造体タグを使用して、Ginがスライス/配列フィールドのリスト値を分割する方法を制御できます。

サポートされるフォーマット（v1.11以降）：
...
コレクションのデフォルト値（v1.11以降）：

- `form`タグの`default`を使用してフォールバック値を設定します。
- `multi`と`csv`では、デフォルト値をセミコロンで区切ります：`default=1;2;3`。
- `ssv`、`tsv`、`pipes`では、デフォルト内で自然な区切り文字を使用します。

関連項目：「フォームフィールドのデフォルト値バインド」の例。
