...
Gin 不包含内置的 WebSocket 实现，但可以与 [gorilla/websocket](https://github.com/gorilla/websocket) 包无缝集成。由于 Gin 处理函数接收底层的 `http.ResponseWriter` 和 `*http.Request`，你可以用最少的代码将任何 Gin 路由升级为 WebSocket 连接。
...
- [自定义 HTTP 配置](/zh-cn/docs/server-config/custom-http-config/) -- 微调底层 HTTP 服务器
