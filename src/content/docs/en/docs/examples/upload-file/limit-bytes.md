...
title: "Limit bytes"
---
#### Restrict file upload size using Gin + http.MaxBytesReader

This example shows how to use Gin together with `http.MaxBytesReader` to safely and *strictly* limit the maximum size of uploaded files. It returns a custom error message if the file is too large.

Source: [Read More](https://github.com/gin-gonic/examples/blob/master/upload-file/limit-bytes/main.go)

#### How it works

1.  **Define Limit**: A constant `MaxUploadSize` (1MB) is defined to set the hard limit for uploads.
...
3.  **Parse & Check**: `c.Request.ParseMultipartForm` triggers the read. If the body is too large, it returns an error. The code specifically checks for `*http.MaxBytesError` to return a `413 Request Entity Too Large` status with a clear message.
