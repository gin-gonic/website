...
Form bağlama ile dilim/dizi alanları için Gin'in liste değerlerini nasıl böldüğünü `collection_format` struct etiketi kullanarak kontrol edebilirsiniz.
...
- pipes: dikey çizgiyle ayrılmış değerler
...
- `ssv`, `tsv` ve `pipes` için, varsayılanlarda doğal ayırıcıyı kullanın.

Ayrıca bakınız: "Form alanları için varsayılan değer bağlama" örneği.
