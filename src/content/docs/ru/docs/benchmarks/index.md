...
> **Обновлённые данные:** Эти бенчмарки были собраны в марте 2026 года с использованием Gin v1.12.0 и Go 1.25.8. Для актуальных результатов бенчмарков смотрите репозиторий [go-http-routing-benchmark](https://github.com/gin-gonic/go-http-routing-benchmark).
...
Бенчмарки ниже показывают результаты различных Go-фреймворков при выполнении типичных задач HTTP-маршрутизации.
**Меньшие числа (время, память, аллокации) лучше.**
Вы можете использовать эти результаты для прямого сравнения Gin с альтернативными маршрутизаторами.

| Тест                              | Повторения | Время (ns/op) | Байты (B/op) | Аллокации (allocs/op) |
| ---------------------------------- | ----------- | ------------ | ------------ | ----------------------- |
...
- **Повторения**: Общее количество повторений, достигнутых за постоянное время. Большие числа означают большую уверенность в результатах.
- **Время (ns/op)**: Продолжительность одной операции в наносекундах. Меньше — лучше.
- **Байты (B/op)**: Память кучи, выделенная на операцию. Меньше означает лучшую эффективность.
- **Аллокации (allocs/op)**: Среднее количество выделений памяти на операцию. Меньше аллокаций — лучше для производительности и сборки мусора.

По вопросам и предложениям обращайтесь в наш [репозиторий GitHub](https://github.com/gin-gonic/gin).
