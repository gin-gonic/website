...
`collection_format` 구조체 태그를 사용하여 폼 바인딩에서 Gin이 슬라이스/배열 필드의 목록 값을 분할하는 방식을 제어할 수 있습니다.

지원되는 형식 (v1.11+):

- multi (기본값): 반복 키 또는 쉼표로 구분된 값
- csv: 쉼표로 구분된 값
- ssv: 공백으로 구분된 값
- tsv: 탭으로 구분된 값
- pipes: 파이프로 구분된 값
...
- `ssv`, `tsv`, `pipes`의 경우, 기본값에 자연 구분자를 사용합니다.

참고: "폼 필드에 대한 기본값 바인딩" 예제도 확인하세요.
